...
// 	_, err := NewPool(USDC, entities.WIP9[3], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil)
// 	assert.ErrorIs(t, err, entities.ErrDifferentChain, "cannot be used for tokens on different chains")

// 	_, err = NewPool(USDC, entities.WIP9[1], 1e6, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil)
...
// 	_, err = NewPool(USDC, entities.WIP9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 1, nil)
// 	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

// 	_, err = NewPool(USDC, entities.WIP9[1], constants.FeeMedium, new(big.Int).Add(utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(1)), big.NewInt(0), -1, nil)
// 	assert.ErrorIs(t, err, ErrInvalidSqrtRatioX96, "price must be within tick price bounds")

// 	_, err = NewPool(USDC, entities.WIP9[1], constants.FeeMedium, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil)
// 	assert.NoError(t, err, "works with valid arguments for empty pool medium fee")

// 	_, err = NewPool(USDC, entities.WIP9[1], constants.FeeLow, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil)
// 	assert.NoError(t, err, "works with valid arguments for empty pool low fee")

// 	_, err = NewPool(USDC, entities.WIP9[1], constants.FeeHigh, utils.EncodeSqrtRatioX96(constants.One, constants.One), big.NewInt(0), 0, nil)
...
// 	_, err := pool.PriceOf(entities.WIP9[1])
...
// 	assert.False(t, pool.InvolvesToken(entities.WIP9[1]), "does not involve WIP9")
// }
